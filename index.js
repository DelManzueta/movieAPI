...
mongoose.connect(process.env.CONNECTION_URI, { useNewUrlParser: true, useUnifiedTopology: true, dbName: 'myFlixDB' });
...
app.get("/movies/:Title/genre", passport.authenticate('jwt', {session: false}), 
    (req, res) => {
...
app.get("/movies/Director/:Name", passport.authenticate('jwt', {session: false}),
    ...
    Movies.findOne({"Director.Name": req.params.Name,})
      .then((movies) => {res.json(movies.Director.Name + ": " + movies.Director.Bio)})
      .catch((err) => {
        console.error(err);
        res.status(500).send("Error: " + err);
      });
